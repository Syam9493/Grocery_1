...
const ProductFilter = () => {
  ...
  const ProductPriceRange = (e) => {
    ...
    const existingPriceFilter = checkItems.find(
  ...
  return (
    <ProductFilterContext.Provider
      value={{ dispatch, removeCheckItem, setCheckItems, checkItems }}
    >
      <aside className="w-64 p-4">
        <h3 className="font-sans text-[25px] font-bold mb-2 border-b-4 border-gray-400">
          Filter Options
        </h3>
        ...
        <form className="h-60 m-4 flex flex-col items-start gap-4 overflow-y-auto">
          {productNames.map((item) => (
            <label key={item.type} className="inline-flex items-center gap-2">
              <input
                type="checkbox"
        ...
        <div className="mb-4 border-b border-gray-400">
          <div className="mb-3">
            ...
            />
            <div className="font-semibold text-2xl mt-1 text-gray-500">
              ₹{priceRange}
            </div>
          </div>
        </div>
